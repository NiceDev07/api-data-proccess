...
def get_pg_engine_factory(db_name: str):
    """Async engine PostgreSQL compartido del lifespan — inyectado vía Depends()."""
    def _get_engine(request: Request) -> AsyncEngine:
        return request.app.state.pg_engines[db_name]
    return _get_engine
...
get_async_engine_callb     = get_pg_engine_factory("callb")
